...
class Renderer: NSObject, MTKViewDelegate {
    ...
    init?(metalKitView: MTKView, gameManager: GameManager) {
        ...
        metalKitView.colorPixelFormat = .rgba16Float // Use a HDR pixel format
        ...
        do {
            colorMap = try Renderer.loadTexture(device: device, textureName: "arrows")
        } catch {
    ...
    class func buildRenderPipelineWithDevice(device: MTLDevice,
        ...
        let fragmentFunction = library?.makeFunction(name: "additive_fragment_main")
    ...
    func draw(in view: MTKView) {
        ...
        if let commandBuffer = commandQueue.makeCommandBuffer() {
            ...
            if let renderPassDescriptor = renderPassDescriptor, let renderEncoder = commandBuffer.makeRenderCommandEncoder(descriptor: renderPassDescriptor) {
                renderPassDescriptor.colorAttachments[0].texture = colorMap
                renderPassDescriptor.colorAttachments[0].loadAction = .clear
                renderPassDescriptor.colorAttachments[0].clearColor = MTLClearColorMake(0.5, 0.5, 0.5, 0.5)
