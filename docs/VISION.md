...
## 1. The Goal
Claude code porting a casual iOS game to an industrial-strength engine. The objective is not just to "port a game," but to architect a **Host-Guest System** that decouples simulation from presentation, enabling native performance in a browser environment.
